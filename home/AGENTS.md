...
--share`; do not configure Tailscale Serve by hand.

## Fork workflow

Forks use two remotes: `origin` (the fork) and `upstream` (the source repo).
Never push to `upstream`. Local `main` is the fork's source of truth.
Mark durable work with `aamini/*` bookmarks. Push those bookmarks to origin.
Sync from upstream by merge. Never rebase `main` onto upstream.
...
Moving `main` backwards or sideways needs user approval first.
A tree-wide format commit stays out of merges. During a merge, resolve
format-only files to upstream. After `main` moves, re-run the formatter as a
fresh tip. Do not put feature commits on top of a format tip.
